...
const CalcObj = {
    exceptionCheck: function () {
        ...
        if (!(Number(document.getElementById("successRate").value) >= 0 && Number(document.getElementById("successRate").value) <= 100)
            ...
            || Number(document.getElementById("wishCount").value) < 0 // 성공 횟수가 0이 될 수 있음
    ...
    calcProbability: function () {
        ...
        // (1) 계산확률
        let calcResult = (MathObj.binomialProbability(successRate, tryCount, wishCount + 1) + MathObj.binomialProbability(successRate, tryCount, wishCount)) / 2;
        if (calcResult <= 0.5) {
            document.getElementById("calcResult").innerText = "상위 " + (Math.ceil(calcResult * 100 * 1000000) / 1000000);
        } else {
            document.getElementById("calcResult").innerText = "하위 " + (Math.ceil((1 - calcResult) * 100 * 1000000) / 1000000);
        }
        
        let subCalcResult1 = MathObj.binomialProbability(successRate, tryCount, wishCount + 1);
        document.getElementById("subCalcResult1").innerText = Math.ceil(subCalcResult1 * 100 * 100000) / 100000;
        let subCalcResult2 = 1 - MathObj.binomialProbability(successRate, tryCount, wishCount);
        document.getElementById("subCalcResult2").innerText = Math.ceil(subCalcResult2 * 100 * 100000) / 100000;
        let subCalcResult3 = MathObj.binomialProbability(successRate, tryCount, wishCount) - MathObj.binomialProbability(successRate, tryCount, wishCount + 1);
        document.getElementById("subCalcResult3").innerText = Math.ceil(subCalcResult3 * 100 * 100000) / 100000;
